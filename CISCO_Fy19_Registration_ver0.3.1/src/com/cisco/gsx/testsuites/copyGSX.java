...
public class copyGSX {
	...
	public void PrimaryupdateReservation_SecondaryCancel(LinkedList<?> pojo1) throws Exception {
		for (int a = 0; a < pojo1.size(); a++) {
			...
			GSX_HotelReservation.selectDates(pojo);
			//Select Hotel - Step3
			if(!pojo.getRegCode().equalsIgnoreCase("EIDHoliday")){
				System.out.println("*************** Not EIDHoiliday************************");
				GSX_HotelReservation.validateCheckinCheckoutInfo();
				GSX_HotelReservation.selectaHotel(pojo);
	...
	@Test(dataProvider = "data-provider", dataProviderClass = Dataproviderclass.class,invocationCount = 1,priority=7)
